...
Tags: share, follow, sidebar, shortcode, widget, social networking, facebook, twitter, google +, pinterest, linkedin
...
* Optional subscription to different icon sets via a content delivery network (CDN), for choice of icon sets and improved website speed.  Currently 47 icon sets to choose from
* Can have Pinterest icon in bottom right of any image inserted into the content via the wordpress media library or ones with the class of "pin-it" so that individual image sharing is possible

People time and again have said how much they enjoy using this plugin and how it does exactly what they want with the minimum of fuss or difficult configuration.  A very simple bookmarking solution that offers a discrete clean layout that does not brand your website in it's image.

Online demo and support site here [example link](http://share-and-follow.com/wordpress-plugin/). Currently icons from either the default set, or from the shiny circle set of icons.
...
We were often requested to make more icon sets available to users of the plugin.  We wanted to do this, and allowed people to replace the icons within the directory structure from the very start, but now we are moving to sprites for speed, those options are not available.
...
So we have responded in the best way we know.  Rather than getting people to download stuff, or un-zip things, or even FTP, we thought we would cut out all of that horribleness by implementing a Content Delivery Network, where all the files for the icons are actually located on
...
No definitely not, it is an optional extra that we have developed and invested in so that you can have more choice. After all it was what you the users asked for.
...
= The facebook share count does not change until I click like, why? =

Facebook does not have the most reliable system for share count. It's their official button, so please direct your upset at them.
...
YES, 100% true.  If just 1 in 100 users signup, their subscriptions will be made lifetime.  No more yearly charging.
...
If this does not work then it could be an ownership problem where your FTP username does not give the webserver access to the files.  This can only be fixed if you have SSH capability, which most people do not.  If you don't ask your hosting company to sort it out, if you do run this command from the share-and-follow/css direcotry

chown wwwrun *

If you are running IIS then you need to make sure that the iUSER has access to the files to write them
...
= The follow tab does not work, I can see it but I cannot click it =

Argh! Your theme programmer was not very clever, they decided to use z-index on your themes container div.  It will either be in the CSS hopefully, or in the HTML.  Delete it if you can.  Otherwise make the follow tab have a higher z-index via CSS.
...
interim release to deal with missing sprites while I address speed issues.

= 1.80.2 =
reduced number of tags to get it back in to the plugin directory.

= 1.80.1 =
Added Pinterest sharing via share button, interactive share and also via on image sharing.

= 1.70.3 =
deleted google buzz for sharing (kept for following for now), mixx for sharing, fixed show_interactive_links() for stumble, improved bottom interactive buttons

= 1.70.2 =

force re-release due to SVN errors and 3 hours without completing commit

= 1.70.1 =

Added a bunch of new social networks including Pinterest and Google plus.  Also now up to 47 sets on the CDN
